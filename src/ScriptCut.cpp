...
int main(int, char **)
{
    ...
    GLFWwindow *window = glfwCreateWindow(1280, 720, "ScriptCut", NULL, NULL);
    if (window == NULL)
        return 1;
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        ImVec2 size(width, height);
        {
            ...
            ImGui::SetNextItemOpen(CommandsOpen);
            if (ImGui::CollapsingHeader("Commands", ImGuiTreeNodeFlags_SpanFullWidth))
            {
                CommandsOpen = !ImGui::IsWindowCollapsed();
                {
                    ImGui::BeginChild("Inside", ImVec2(0, height - 60), true, ImGuiWindowFlags_AlwaysAutoResize);
                    ImGui::Text("Commands here!");
                    ImGui::EndChild();
                }
            }
